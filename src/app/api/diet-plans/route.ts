...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = session.isAdmin
      ? await dbQuery<any[]>('SELECT * FROM diet_plans')
      : await dbQuery<any[]>('SELECT * FROM diet_plans WHERE user_id = ?', [session.userId]);
    ...
    console.error('Erro GET /api/diet-plans:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('Erro POST /api/diet-plans:', error);
    return NextResponse.json({ error: 'Erro interno.' }, { status: 500 });
  }
}
